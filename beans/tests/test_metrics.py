...
from beans.beans.metrics import (
    Accuracy,
    AveragePrecision,
    BinaryF1Score,
...
def test_accuracy():
    ...
    assert metric.get_metric() == {"acc": 1.0}
    metric.update(
        torch.tensor([[1.0, 0.0, 0.0]]), torch.tensor([[3]])
...
def test_multiclass_binary_f1score():
    ...
    assert metric.get_metric() == pytest.approx(
        {"macro_prec": 7 / 8, "macro_rec": 7 / 8, "macro_f1": 5 / 6}
    )
...
def test_mean_average_precision():
    ...
    target = torch.Tensor([0, 1, 0, 1])
    output = torch.Tensor([0, 1, 0.5, 0.5])
    metric.reset()
    ...
    output = torch.Tensor([[0.1, 0.2, 0.3, 4], [0, 1, 0.5, 0.5]]).transpose(0, 1)
    metric.update(output, target)
