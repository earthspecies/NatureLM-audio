...
from beans.beans.utils import divide_annotation_to_chunks, divide_waveform_to_chunks
...
def test_divide_annotation_to_chunks():
    annotations = [{"st": 0.1, "ed": 0.2}, {"st": 0.8, "ed": 1.2}, {"st": 2.8, "ed": 4.2}]
    ...
    assert chunks[1] == [{"st": pytest.approx(0.0), "ed": pytest.approx(0.2)}]
    assert chunks[2] == [{"st": pytest.approx(0.8), "ed": pytest.approx(1.0)}]
    assert chunks[3] == [{"st": pytest.approx(0.0), "ed": pytest.approx(1.0)}]
    assert chunks[4] == [{"st": pytest.approx(0.0), "ed": pytest.approx(0.2)}]
