...
local["wget"][
    ...
    "-O",
    "data/hainan_gibbons/Train_Labels.zip",
] & FG
local["unzip"][
    "data/hainan_gibbons/Train_Labels.zip", "-d", "data/hainan_gibbons/"
] & FG
...
for file_id, wav_path in enumerate(
    sorted(Path("data/hainan_gibbons/").glob("Train/*.wav"))
):
    ...
    df = pd.read_csv(
        str(wav_path.parent.parent / "Train_Labels" / ("g_" + wav_path.stem)) + ".data"
    )
    annotations = []
