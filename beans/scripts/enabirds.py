...
local["wget"][
    "https://datadryad.org/stash/downloads/file_stream/641808",
    "-O",
    "data/enabirds/wav_Files.zip",
] & FG
local["unzip"]["data/enabirds/wav_Files.zip", "-d", "data/enabirds/"] & FG

local["wget"][
    "https://datadryad.org/stash/downloads/file_stream/641805",
    "-O",
    "data/enabirds/annotation_Files.zip",
] & FG
...
for wav_path in sorted(Path("data/enabirds/").glob("Recording_?/*.wav")):
    ...
    df = pd.read_csv(
        str(wav_path.parent / wav_path.stem) + ".Table.1.selections.txt", sep="\t"
    )
