...
local["wget"][
    "https://archive.org/download/egyptian_fruit_bats_10k/egyptian_fruit_bats.zip", "-P", "data/egyptian_fruit_bats"
] & FG
...
df = df.apply(
    lambda r: pd.Series({"path": f"data/egyptian_fruit_bats/audio/{r['File Name']}", "label": r["Emitter"]}), axis=1
)
...
df_valid, df_test = train_test_split(
    df_valid_test, test_size=0.5, random_state=42, shuffle=True, stratify=df_valid_test["label"]
)
